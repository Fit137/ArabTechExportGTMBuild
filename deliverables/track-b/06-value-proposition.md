# Track B, Phase 6. Value Proposition

Steps 6.1 and 6.2. Three pillars, nine reasons to believe, nine copy drills.

**Two fields are blocked in all nine drills** per `SKILL-ADAPTATION.md`. Headline
(Social Proof) and Supporting Proof Point are `{TBD}`, because Arab Tech Export has
no customers, no results and no testimonials. Eighteen fields, all empty, all
deliberately. Where the skill asks for social proof, a method line is given instead:
what the approach is and what it demands of the member. That signals seriousness
without claiming a result.

**Language note.** Copy below is in English, which is the working language of this
document. Per `00-ASSUMPTIONS.md` decision 1, top of funnel headlines ship in
Arabic. Write them in Arabic, do not translate these. A translated headline carries
...
## Step 6.1. Brand promise and positioning
...
## Step 6.2, Table 1. Value proposition pillars

The three pillars map to the three roles in the buyer's internal argument, see
Phase 5. The user, the manager, and the decision maker each get a pillar, because
each is blocking for a different reason.
...
category with justified trust problems. Published terms are a stronger answer to
that objection than any claim, and this business has no claims available.

---

## Copy drills

Nine tables. Body copy is capped at 25 words per the skill.

### Drill 1. VP1, RTB 1.1. Buyer evaluation walkthroughs
...
### Drill 2. VP1, RTB 1.2. The readiness assessment
...
### Drill 3. VP1, RTB 1.3. Marked up proposals
...
### Drill 4. VP2, RTB 2.1. Live deal teardowns
...
### Drill 5. VP2, RTB 2.2. Positioning review
...
| Subhead | The profile, the first message, the one line that decides whether the rest is read |
| Body copy | A buyer decides whether to keep reading in one line. Most Arab builders write that line for a buyer who already knows them. |
...
| Element | Content |
|---|---|
| Reason to believe | A founding cohort of 8 to 12, recruited individually |
| Headline (Rational) | Eight to twelve people. Recruited one at a time |
| Headline (Emotional) | A room where somebody notices you did not show up |
| Headline (Social Proof) | `{TBD}`. **Method substitute:** Capped at twelve, by invitation, because a room that anyone can join is a room nobody is accountable in |
| Subhead | Small on purpose. Everyone in it has a live deal and everyone is visible |
| Body copy | Most paid communities lose a third of members in two weeks because nobody noticed they stopped posting. Twelve people notice. |
| CTA | Apply to the founding cohort |
| Supporting proof point | `{TBD}` |

### Drill 8. VP3, RTB 3.2. Founder access

| Element | Content |
|---|---|
| Reason to believe | Direct founder access, capped by design and priced accordingly |
| Headline (Rational) | One to one time, with a fixed number of slots |
| Headline (Emotional) | The thing you actually wanted, and it is not the course |
| Headline (Social Proof) | `{TBD}`. **Method substitute:** Slots are capped and published. When they are gone the tier closes rather than quietly degrading |
| Subhead | Limited, because the alternative is promising access that stops arriving |
| Body copy | Direct access is the most praised thing in this category and the first to disappear at scale. The slot count is published for that reason. |
| CTA | See remaining slots |
| Supporting proof point | `{TBD}` |

### Drill 9. VP3, RTB 3.3. Published terms

| Element | Content |
|---|---|
| Reason to believe | Stated cancellation and refund terms, published before purchase |
| Headline (Rational) | The cancellation terms, before you pay, not after |
| Headline (Emotional) | You have been burned by this category before |
| Headline (Social Proof) | `{TBD}`. **Method substitute:** Terms published on the pricing page. No discretion, no case by case, no messaging anyone to ask |
| Subhead | Cancel from the account page. No retention call |
| Body copy | In this category refunds are usually at the seller's discretion. Ours are written down and you can read them before you decide. |
| CTA | Read the terms |
| Supporting proof point | `{TBD}` |
...
not need to be impressive, it needs to be real and specific.

The nine social proof headlines stay `{TBD}` until there are members with
permission to be quoted, which by definition is after the founding cohort has run.
Design partner success metrics in Phase 5 include exactly that permission, which is
where these nine get filled from.

Do not let a copywriter fill these later without reading
`GTM-BRIEF.md` Section 5. Nine empty social proof fields in a template is the most
likely place in this entire run for a fabricated testimonial to enter.
